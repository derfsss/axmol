...
struct LineBreakLine
{
    ...
    void calculateWidth()
    {
        ...
        if (!glyphs.empty())
        {
            LineBreakGlyph& glyph = glyphs.back();
...
int Device::getDPI()
{
    // AmigaOS4: return a sensible default. Could query Intuition screen in the future.
    return 72;
}
...
public:
    ...
    int utf8(char** p)
    {
        if ((**p & 0x80) == 0x00) { int a = *((*p)++); return a; }
        if ((**p & 0xE0) == 0xC0) { int a = *((*p)++) & 0x1F; int b = *((*p)++) & 0x3F; return (a << 6) | b; }
        if ((**p & 0xF0) == 0xE0) { int a = *((*p)++) & 0x0F; int b = *((*p)++) & 0x3F; int c = *((*p)++) & 0x3F; return (a << 12) | (b << 6) | c; }
        if ((**p & 0xF8) == 0xF0) { int a = *((*p)++) & 0x07; int b = *((*p)++) & 0x3F; int c = *((*p)++) & 0x3F; int d = *((*p)++) & 0x3F; return (a << 18) | (b << 12) | (c << 8) | d; }
        return 0;
    }
    ...
    std::string getFontFile(const char* family_name)
    {
        ...
        auto it = fontCache.find(family_name);
        if (it != fontCache.end())
            return it->second;
        ...
        // No fontconfig on AmigaOS4 — return as-is
        return family_name;
    }
    ...
    bool getBitmap(const char* text, const FontDefinition& textDefinition, Device::TextAlign eAlignMask)
    {
        ...
        for (int line = 0; line < (int)textLines.size(); line++)
        {
            ...
            for (int i = 0; i < (int)textLines.at(line).glyphs.size(); i++)
            {
...
void Device::prepareImpactFeedbackGenerator(ImpactFeedbackStyle) {}
void Device::impactOccurred(ImpactFeedbackStyle) {}
void Device::prepareNotificationFeedbackGenerator() {}
void Device::notificationOccurred(NotificationFeedbackType) {}
